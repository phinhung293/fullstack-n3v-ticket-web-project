...
import { Ticket, Clock, CheckCircle2, XCircle, AlertCircle, Search, Filter, CalendarDays, UserRound } from 'lucide-react';
...
export default function AdminOrders() {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const res = await axiosInstance.get<ApiResponse<AdminOrderResponse[]>>('/admin/orders');
                setOrders(res.data.data);
            } catch (err: any) {
                setError(err.response?.data?.message || 'Có lỗi xảy ra khi tải danh sách đơn vé');
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <section className="rounded-2xl bg-white p-6 shadow-[0_12px_30px_rgba(15,23,42,0.06)] min-h-[500px] h-fit">
            <h1 className="text-xl font-black text-[#0B1736]">Quản lý Đơn vé</h1>
            <p className="mt-1 text-sm font-medium text-[#64748B]">Danh sách toàn bộ đơn hàng trong hệ thống</p>
            ...
            <div className="mt-6 space-y-4 pr-2 custom-scrollbar overflow-y-auto">
                    ...
                    return (
                        <div key={order.orderCode} className="flex flex-col gap-5 rounded-2xl border border-[#E2E8F0] bg-white p-5 shadow-sm transition hover:border-[#CBD5E1] sm:flex-row sm:items-center sm:justify-between">
                            <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm font-medium text-[#475569]">
                                    <UserRound size={14} className="text-[#94A3B8]" />
                    ...
                    <div className="relative w-full max-w-[500px] rounded-3xl bg-white p-6 shadow-2xl sm:p-8 max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="mt-8 space-y-4 rounded-2xl bg-[#F8FAFC] p-5">
                            <div className="flex justify-between border-b border-[#E2E8F0] pb-4">
                                ...
                                <span className="text-sm font-bold text-[#0B1736] text-right">{selectedOrder.customerName || 'Khách vãng lai'}</span>
                            </div>
                            <div className="flex justify-between border-b border-[#E2E8F0] pb-4">
                                <span className="text-sm font-medium text-[#64748B]">Email / SĐT</span>
                                <span className="text-sm font-bold text-[#0B1736] text-right">{selectedOrder.customerEmail || ''} {selectedOrder.customerPhone ? `(${selectedOrder.customerPhone})` : ''}</span>
                            </div>
                            <div className="flex justify-between border-b border-[#E2E8F0] pb-4">
